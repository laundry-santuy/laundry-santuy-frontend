...
export function HistoryLoadingState() {
  return (
    <section
      aria-label="Memuat riwayat order"
      className="mx-auto grid w-full max-w-[1440px] gap-5 xl:grid-cols-[minmax(0,1fr)_400px]"
    >
      <div className="space-y-5">
        <div className="h-[230px] animate-pulse rounded-[32px] bg-[var(--odong-surface)]" />
        <div className="h-[92px] animate-pulse rounded-[28px] bg-[var(--odong-surface)]" />
        <div className="h-[560px] animate-pulse rounded-[32px] bg-[var(--odong-surface)]" />
      </div>
      <div className="h-[560px] animate-pulse rounded-[32px] bg-[var(--odong-surface)]" />
      <span className="sr-only">Memuat data riwayat</span>
    </section>
  );
}
...
export function HistoryErrorState() {
  return (
    <HistoryStateCard
      ...
      description="Coba muat ulang halaman atau cek koneksi sebelum melihat daftar transaksi."
    />
  );
}
...
export function HistoryEmptyState() {
  return (
    <HistoryStateCard
      ...
      description="Transaksi laundry yang selesai, aktif, atau dibatalkan akan muncul di halaman ini."
    />
  );
}
