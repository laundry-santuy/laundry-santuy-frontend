...
import { profileAddresses, userProfile } from "./data";
import {
  ProfileEmptyState,
  ProfileErrorState,
  ProfileLoadingState,
...
export function ProfilePage({ status = "ready" }: ProfilePageProps) {
  const primaryAddress = profileAddresses.find((address) => address.isPrimary);
      ...
      <div className="relative z-10 space-y-5 pb-10 sm:pb-12">
        ...
        <section className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)] 2xl:grid-cols-[380px_minmax(0,1fr)]">
          <div className="space-y-5">
            <ProfileSummaryCard />
            <AddressCard address={primaryAddress ?? profileAddresses[0]} />
          </div>
        ...
        <span className="sr-only" aria-live="polite">
          Profil {userProfile.name} terbuka.
        </span>
      </div>
    </div>
  );
...
type AddressCardProps = {
  address: (typeof profileAddresses)[number];
};
  ...
  return (
    <section className="rounded-[32px] border border-[var(--odong-border)] bg-[var(--odong-surface)] p-6 shadow-[0_18px_46px_rgba(25,28,29,0.07)] backdrop-blur-xl">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          type="button"
          className="inline-flex h-10 items-center justify-center gap-2 rounded-2xl border border-primary-100 bg-primary-50 px-3 text-sm font-bold text-primary-700 transition hover:-translate-y-0.5 hover:bg-primary-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-300 active:scale-[0.98]"
        >
          ...
          return (
            ...
            >
              <span className="flex min-w-0 items-center gap-4">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-primary-50 text-primary-600 transition group-hover:bg-white">
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-1 block text-sm leading-6 text-[var(--odong-muted)]">
                    {item.description}
                  </span>
                </span>
              </span>
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/70 text-[var(--odong-muted)] transition group-hover:text-primary-700">
...
function LogoutPanel() {
  return (
    <Link
      ...
      className="group flex items-center justify-between gap-4 rounded-[28px] border border-red-100 bg-red-50/80 p-5 text-left shadow-[0_14px_34px_rgba(220,38,38,0.08)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-200 active:scale-[0.99]"
    >
      <span className="flex min-w-0 items-center gap-4">
        <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white text-red-600">
          <LogOut className="h-5 w-5" aria-hidden="true" />
        </span>
        <span className="min-w-0">
          <span className="block text-base font-extrabold text-red-700">
            Keluar
          </span>
          <span className="mt-1 block text-sm leading-6 text-red-500">
            Akhiri sesi dan kembali ke halaman login.
          </span>
        </span>
      </span>
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/80 text-red-500 transition group-hover:text-red-700">
        <ChevronRight className="h-4 w-4" aria-hidden="true" />
      </span>
    </Link>
  );
