...
import type { DriverActiveOrderFilter } from "../types";

type ActiveOrdersToolbarProps = {
  searchQuery: string;
  activeFilter: DriverActiveOrderFilter;
  resultCount: number;
  filters: DriverActiveOrderFilter[];
  onSearchChange: (value: string) => void;
  onFilterChange: (filter: DriverActiveOrderFilter) => void;
};
    ...
    <section className="fixed bottom-6 right-6 z-50 sm:bottom-8 sm:right-8">
      <Popover>
        <PopoverTrigger asChild>
          <button
            type="button"
            aria-label="Buka filter pesanan aktif"
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs font-medium text-[var(--odong-muted)]">
                {resultCount} order ditemukan - {activeFilter}
              </p>
            </div>
            <button
              type="button"
          ...
          <label className="relative mt-4 block min-w-0">
            <span className="sr-only">Cari pesanan aktif</span>
            <Search
            ...
            <input
              ...
              placeholder="Cari order, nama, alamat..."
